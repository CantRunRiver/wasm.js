...
export default class Instruction extends _Base {
	 ...
	 * 
	 */
	public "opname": Constants.Opname;
	 ...
	 * 
	 */
	public "opcode": typeof Constants.Opcode[keyof typeof Constants.Opcode];
	...
	override read() {
		...
		this.opname = Object.keys(Constants.Opcode)[Object.values(Constants.Opcode).indexOf(this.opcode)] as Constants.Opname;
		switch (this.opname) {
			...
			case "i64.const": {
				const value = this.reader.VarUint64();
				this.immediates.value = value;
				break;
			}
			case "f32.const": {
